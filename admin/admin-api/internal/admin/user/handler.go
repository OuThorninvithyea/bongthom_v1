...
func (h *UserHandler) Show(c fiber.Ctx) error {
	...
	return c.Status(fiber.StatusOK).JSON(
		response.NewResponseWithPaing(msg, constants.Generic_success, users.Users, usersShowRequest.PageOption.Page, usersShowRequest.PageOption.Perpage, users.Total),
	)
}
...
func (h *UserHandler) Update(c fiber.Ctx) error {
	...
	user, e := h.Service.Update(id, req, uCtx)
	if e != nil {
		msg, e_msg := translate.TranslateWithError(c, e.MessageID)
		if e_msg != nil {
			return c.Status(fiber.StatusBadRequest).JSON(
...
func (h *UserHandler) Delete(c fiber.Ctx) error {
	...
	if e := h.Service.Delete(id, uCtx); e != nil {
		msg, e_msg := translate.TranslateWithError(c, e.MessageID)
		if e_msg != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(
				response.NewResponseError(e_msg.Err.Error(), constants.Translate_Failed, e_msg.Err),
